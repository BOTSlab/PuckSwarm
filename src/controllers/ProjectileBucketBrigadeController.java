...
/**
 * Initial prototype of projectile bucket brigading.  Forked from BucketBrigadeController.
 */
public class ProjectileBucketBrigadeController implements KickingController, PropertiesListener {
	...
	public ProjectileBucketBrigadeController() {
		propertiesUpdated();
	...
	public void propertiesUpdated() {
		...
		PICK_UP_TIME = e.getProperty(
				"ProjectileBucketBrigadeController.PICK_UP_TIME", 20, this);
		BACKUP_TIME = e.getProperty(
				"ProjectileBucketBrigadeController.BACKUP_TIME", 5, this);
		POST_DROP_TIME = e.getProperty(
				"ProjectileBucketBrigadeController.POST_DROP_TIME", 5, this);
	}
	...
	public void computeDesired(Suite suite, int stepCount) {
		...
		switch (state) {
			...
			case DE_HOMING:
				...
				else if (BucketBrigadeController.seeRobotForPass(robotPose, localMap)) {
	...
	public String getInfoString() {
		return "ProjectileBucketBrigade: " + state;
	}
